...
struct Queue
{
    int *data;       //the array which has elements
...
int noOfItems(struct Queue *queue)  // to get the total number of elements present in the queue
{
...
int isFull(struct Queue *queue)    // checks if the queue is full or not
{
...
int enQueue(struct Queue *queue, int item) //insert element into the queue
{
    printf("enQueue %d:\n", item);
